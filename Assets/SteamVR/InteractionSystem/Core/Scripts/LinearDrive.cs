...
namespace Valve.VR.InteractionSystem
{
	...
	public class LinearDrive : MonoBehaviour
	{
		...
		public float speedToNeutralPosition = 0;
		...
		protected virtual void Update()
        {
			...
			if (speedToNeutralPosition > 0 && linearMapping.value != neutralPosition) {
				float delta = linearMapping.value - neutralPosition;
				linearMapping.value = Mathf.Clamp01( linearMapping.value - (Mathf.Clamp(delta, -speedToNeutralPosition, speedToNeutralPosition)  * Time.deltaTime ) );
				if ( repositionGameObject ) {
					transform.position = Vector3.Lerp( startPosition.position, endPosition.position, linearMapping.value );
				}
			}
